...
__host__ void generate_rand_data(unsigned int * data, unsigned int num_elements)
{
    ...
    {
        data[i] = rand() % num_elements; // random enough 
    }
}
...
__host__ float execute_shared_memory_operations()
{
    ...
    cudaStreamDestroy(stream);
    // cudaFreeHost(hi_data);               //free up the host memory   this should not be freed?  
    // cudaFreeHost(hf_data);               //free up the host memory   they are stack allocted 
    cudaDeviceReset();
